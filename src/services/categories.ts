...
export async function getCategories(userId: string): Promise<Category[]> {
  ...
  if (!db) {
    ...
    try {
      const parsed = JSON.parse(localStr) as Record<string, Category[]>;
      return parsed[userId] ?? DEFAULT_CATEGORIES;
    } catch {
...
export async function saveCategories(userId: string, categories: Category[]): Promise<void> {
  ...
  if (!db) {
    ...
    if (localStr) {
      try {
        all = JSON.parse(localStr) as Record<string, Category[]>;
      } catch {
        // Ignore invalid local data and recreate the structure.
      }
    }
